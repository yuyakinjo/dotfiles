...
# 実行時のカレントディレクトリによって動作を分岐する:
#   - リポジトリ(chezmoi source-path)の中で実行 → chezmoi apply でデプロイ先に反映してからcommit・push
#   - リポジトリの外で実行                     → chezmoi re-add でデプロイ先の変更を取り込んでからcommit・push
# 使い方:
...
function dotfiles_sync() {
  ...
  if [[ "$PWD" == "$source_dir" || "$PWD" == "$source_dir"/* ]]; then
    print "▶︎ リポジトリ内の変更をchezmoi applyでデプロイ先に反映中..."
    chezmoi apply
  else
